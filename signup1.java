...
public class Signup2 extends JFrame implements ActionListener{
    ...
    if (pan.isEmpty() || aadhar.isEmpty() || religion.isEmpty() || category.isEmpty() || 
        income.isEmpty() || education.isEmpty() || occupation.isEmpty()) {
        
        JOptionPane.showMessageDialog(null, "Fill all the required fields", "Warning", JOptionPane.WARNING_MESSAGE);
        return;
    }

    // PAN Validation - Must be exactly 10 alphanumeric characters
    if (!pan.matches("[A-Z0-9]{10}")) {
        JOptionPane.showMessageDialog(null, "PAN Number must be exactly 10 alphanumeric characters", "Invalid PAN", JOptionPane.WARNING_MESSAGE);
        return;
    }

    // Aadhar Validation - Must be exactly 12 digits
    if (!aadhar.matches("\\d{12}")) {
        JOptionPane.showMessageDialog(null, "Aadhar Number must be exactly 12 digits", "Invalid Aadhar", JOptionPane.WARNING_MESSAGE);
        return;
    }

    // Proceed with database entry if all validations pass
    try {
        Conn c1 = new Conn();
